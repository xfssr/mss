    ...
    return (
      <div className="mt-8 flex flex-col sm:flex-row items-center gap-3">
        <button
          ...
          className="inline-flex items-center justify-center rounded-xl border border-[#C6A75E]/30 bg-[#C6A75E]/10 px-6 py-3 text-sm font-medium text-white hover:bg-[#C6A75E]/20 hover:border-[#C6A75E]/50 transition-all duration-200 hover:-translate-y-0.5 shadow-lg"
        >
          {t(lang, "sectionCtaWa")}
        </button>
        <span className="text-xs text-[#B9B5AC]/60">{t(lang, "ctaUrgency")}</span>
      </div>
    );
  }

  return (
    <div className="min-h-dvh-safe bg-gradient-to-b from-[#0A0A0C] via-[#0A0A0C] to-[#0A0A0C] text-white">
        ...
        <div className="relative cc-glass rounded-3xl p-6 sm:p-10 lg:p-12 shadow-2xl">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-10 items-center">
            <div className="lg:col-span-7">
              <h1 className="text-4xl sm:text-5xl lg:text-6xl font-bold text-[#C6A75E] font-serif leading-tight">{t(lang, "heroHeadline")}</h1>
              <p className="mt-6 text-base sm:text-lg text-[#B9B5AC] leading-relaxed max-w-xl">{t(lang, "heroSupporting")}</p>

              <div className="mt-8 flex flex-col sm:flex-row gap-3">
                <button
                  type="button"
                  onClick={onSendWhatsApp}
                  className="inline-flex items-center justify-center rounded-xl border border-[#C6A75E]/40 bg-[#C6A75E]/15 px-7 py-4 text-base font-semibold text-white hover:bg-[#C6A75E]/25 hover:border-[#C6A75E]/60 transition-all duration-200 hover:-translate-y-0.5 shadow-lg hover:shadow-xl hover:shadow-[#C6A75E]/10"
                >
                  {t(lang, "heroCtaWa")}
                </button>
                <a
                  href="#packages"
                  className="inline-flex items-center justify-center rounded-xl border border-white/[0.06] bg-white/[0.04] px-6 py-4 text-sm font-medium text-[#B9B5AC] hover:bg-white/[0.08] hover:border-white/[0.12] transition-all duration-200 hover:-translate-y-0.5"
                >
                  {t(lang, "heroCtaAvailability")}
                </a>
              </div>
              <p className="mt-3 text-xs text-[#B9B5AC]/60">{t(lang, "ctaUrgency")}</p>
            </div>
      ...
      <Section id="packages" title={t(lang, "choosePackage")}>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      {pkg.badge === "popular" ? (
                        <span className="text-[10px] rounded-full border border-[#C6A75E]/30 bg-[#C6A75E]/10 text-[#C6A75E] px-2.5 py-0.5 font-medium shadow-sm">
                          {t(lang, "popular")}
                        </span>
                      ) : null}
                    </div>
                    {price > 0 && (
                      <div className="mt-1.5 flex items-center gap-2 flex-wrap">
                        <span className="text-sm font-semibold text-[#C6A75E]/80">
                ...
                <div className="mt-5 flex items-center gap-3">
                  {!isExpanded && (
                    <button
                      ...
                      className="inline-flex items-center justify-center rounded-xl border border-[#C6A75E]/30 bg-[#C6A75E]/10 px-5 py-2.5 text-sm font-medium text-white hover:bg-[#C6A75E]/20 hover:border-[#C6A75E]/50 transition-all"
                    >
                      {t(lang, "pkgWhatsApp")}
                    </button>
                  )}
              ...
              {detail && isExpanded && (
                <div className="border-t border-white/15 px-6 sm:px-8 py-5 space-y-4 text-sm animate-in slide-in-from-top-2 duration-200">
                  ...
                  {isMonthly && (hasDiscount || addonsTotal > 0) && (
                    <div className="rounded-lg border border-[#C6A75E]/30 bg-[#C6A75E]/10 px-3 py-2">
                  ...
                  <div className="pt-2" style={{ paddingBottom: "env(safe-area-inset-bottom)" }}>
                    <button
                      type="button"
                      onClick={handleWhatsApp}
                      className="w-full inline-flex items-center justify-center rounded-xl border border-[#C6A75E]/30 bg-[#C6A75E]/10 px-4 py-3 text-sm font-medium text-white hover:bg-[#C6A75E]/20 hover:border-[#C6A75E]/50 transition-all"
                    >
                      {t(lang, "pkgWhatsApp")}
                    </button>
                  </div>
        ...
        {props.discountConfig.enabled && (
          <p className="mt-4 text-xs text-[#C6A75E]/70">
          ...
          <ul className="space-y-4">
            {(["whoIsThisFor1", "whoIsThisFor2", "whoIsThisFor3", "whoIsThisFor4", "whoIsThisFor5", "whoIsThisFor6", "whoIsThisFor7"] as const).map((key) => (
              <li key={key} className="flex items-start gap-3 text-sm sm:text-base text-white/80">
                <span className="text-[#C6A75E] mt-0.5 shrink-0">●</span>
                {t(lang, key)}
              </li>
            ))}
          </ul>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 lg:gap-6 max-w-4xl">
          {(["trustStat1", "trustStat2", "trustStat3", "trustStat4"] as const).map((key) => (
            <div key={key} className="trust-stat-card p-5 sm:p-6 text-center">
              <div className="text-2xl sm:text-3xl font-bold text-[#C6A75E]">{t(lang, `${key}Value`)}</div>

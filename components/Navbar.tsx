...
const navLinkClass =
  "px-3 py-2 text-sm text-[#B9B5AC] hover:text-[#F5F3EE] hover:bg-white/5 rounded-lg transition-all focus:outline-none focus-visible:ring-2 focus-visible:ring-[#C6A75E]";
...
export function Navbar(props: { lang: Lang; onSetLang: (l: Lang) => void }) {
  ...
  return (
    <header className="sticky top-0 z-30 border-b border-white/[0.06] bg-[#0A0A0C]/80 backdrop-blur-md sm:backdrop-blur-lg">
      <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
        <a
          href="#top"
          className="flex items-center min-w-0 focus:outline-none focus-visible:ring-2 focus-visible:ring-[#C6A75E] focus-visible:ring-offset-2 focus-visible:ring-offset-black/50 rounded-lg px-2 py-1 transition-opacity hover:opacity-90"
        ...
        <nav className="flex items-center gap-2" aria-label="Main navigation">
          ...
          <button
            type="button"
            className="md:hidden p-2 rounded-lg text-[#B9B5AC] hover:text-[#F5F3EE] hover:bg-white/5 transition-all focus:outline-none focus-visible:ring-2 focus-visible:ring-[#C6A75E]"
          ...
          <div className="ml-2 inline-flex rounded-xl border border-white/[0.06] bg-[#0A0A0C]/60 p-1 backdrop-blur-sm" role="group" aria-label="Language">
            <button
              type="button"
              ...
              className={[
                ...
                isHe ? "bg-[#C6A75E]/15 text-[#C6A75E] shadow-sm" : "text-[#B9B5AC] hover:text-[#F5F3EE] hover:bg-white/[0.06]",
              ].join(" ")}
            >
              עברית
            </button>
      ...
      {mobileOpen && (
        <div className="md:hidden border-t border-white/[0.06] bg-[#0A0A0C]/90 backdrop-blur-lg">
          <div className="flex flex-col px-4 py-2">
            {NAV_ITEMS.map((item) =>
              item.useLink ? (
                <Link
                  key={item.labelKey}
                  className="px-3 py-3 text-sm text-[#B9B5AC] hover:text-[#F5F3EE] hover:bg-white/5 rounded-lg transition-all"
                  href={item.href}
